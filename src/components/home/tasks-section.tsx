...
import { AddTaskCard } from "../sidebar/add-task/add-task-card";
...
const SubGoalsList: React.FC = () => {
      ...
      <li
        ...
        className={`relative flex group items-center w-full border-b gap-3 pl-3 py-2 ${
          isDragging
            ? "opacity-50 "
            : isOver
            ? "bg-violet-50 border-t border-violet-600"
            : "rounded"
        }`}
      >
        <span className="absolute cursor-move py-1 rounded hover:bg-neutral-200 -left-2 top-[calc(50%-12px)] group-hover:block hidden text-muted-foreground">
  ...
  const DroppableSubGoal: React.FC<{
    ...
    return (
      <div
        key={subGoal.id}
        className={`overflow-hidden ${isOver ? "border-blue-500 border" : ""}`}
      >
        <Collapsible open={isOpen} onOpenChange={onToggle}>
          ...
          <CollapsibleContent>
            <CardContent
              ref={contentRef}
              className={`relative pt-4 pl-2 ml-6 border-l-2 ${
                isOver ? "bg-blue-50" : ""
              }`}
            >
              <span className="absolute -left-6 top-[calc(50%-12px)]">t</span>
